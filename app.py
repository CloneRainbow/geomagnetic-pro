"""
Geomagnetic Pro 2025 v5.0 — GeoJSON | GPX | KML | Офлайн-режим | Оптимізований
WMM2025 + RTDM | MGRS/UTM | Автовисота | Теплова карта по точкам | Історія
"""
...
# =============================================
# ОФЛАЙН DEM (кеш у CSV)
# =============================================
DEM_CACHE = os.path.join(CACHE_DIR, "dem_cache.csv")
if os.path.exists(DEM_CACHE):
    _dem_df = pd.read_csv(DEM_CACHE).set_index(["lat", "lon"])["elev"]
else:
    _dem_df = pd.DataFrame(columns=["elev"]).astype(float)

@st.cache_data(ttl=86400)
def get_elevation(lat: float, lon: float) -> float:
    key = (round(lat, 6), round(lon, 6))
    if key in _dem_df.index:
        return float(_dem_df.loc[key])
    try:
        url = f"{DEM_API}?locations={lat},{lon}"
        r = requests.get(url, timeout=10).json()
        elev = r["results"][0]["elevation"]
        if elev is not None:
            _dem_df.loc[key] = elev
            _dem_df.to_csv(DEM_CACHE)
            return round(elev, 1)
    except:
        pass
    return 0.0
...
def get_rtdm(lat: float, lon: float, alt: float = 0) -> Dict:
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            _rtdm_df.loc[key] = [decl_rt, total_rt, storm]
            _rtdm_df.to_csv(RTDM_CACHE)
