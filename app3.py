...
if st.button("Додати точку (lat/lon)", type="secondary", key="add_latlon"):
...
if st.button("Конвертувати MGRS → Додати точку", key="convert_mgrs"):
...
with col_u4:
    if st.button("Конвертувати UTM", key="convert_utm"):
...
def parse_geotiff(file) -> List[Dict]:
    with rasterio.open(file) as src:
        ...
        points = []
        for row in range(0, height, max(1, height // 100)):  # Підвибірка для продуктивності
            for col in range(0, width, max(1, width // 100)):
...
def export_geotiff(df: pd.DataFrame, field: str = "decl") -> bytes:
    ...
    with rasterio.MemoryFile() as memfile:
        with memfile.open(driver='GTiff', height=grid.shape[0], width=grid.shape[1], count=1, dtype=grid.dtype, crs='EPSG:4326', transform=transform) as dataset:
...
st.markdown("**WMM2025 | Пошук місць + Візуалізація + Імпорт/Експорт усіх форматів**")
...
processing.run("native:buffer", {'INPUT': layer, 'DISTANCE': 10000, 'OUTPUT': 'memory:Buffer'})''', language="python")
